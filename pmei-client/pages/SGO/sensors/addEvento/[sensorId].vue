...
<template>
  <div class="min-h-screen bg-gray-100 py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
      ...
      <h1 v-if="sensor && sensorId" class="text-3xl font-bold text-gray-900 mb-8">Adicionar Evento ao Sensor
        {{ sensorId }} - {{ sensor.tipo }}</h1>
      ...
      <form @submit.prevent="createEvento" class="space-y-6 bg-white p-6 rounded-lg shadow">
        ...
        <div v-else>
          <label for="valor" class="block text-sm font-medium text-gray-700">Valor</label>
          <input placeholder="Ex: 39.73451596424165, -8.821039090383264" name="valor" id="valor" v-model="evento.valor"
            type="text"
            class="mt-1 block w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            required>
        </div>
