...
const encomendaId = ref(route.params.idOrder)
...
async function fetchSensors() {
  try {
    // fetch the sensor
    sensors.value = await $fetch(`${api}/encomendas/${encomendaId.value}/sensores`, {
      headers: {
...
async function fetchEncomenda() {
  try {
    // fetch the volume
    encomenda.value = await $fetch(`${api}/encomendas/${encomendaId.value}`, {
      headers: {
        Authorization: `Bearer ${token.value}`
      }
    })
...
const getEstadoSensor = (status) => {
  return status === true ? 'Ativo' : 'Inativo';
}

onMounted(async () => {
  await fetchEncomenda()
  await fetchSensors()
})
</script>
<template>
  <div class="min-h-screen bg-gray-100 p-4">
    <div class="max-w-7xl mx-auto">
      ...
      <div v-if="encomenda" class="bg-white rounded-lg shadow-md p-4 mb-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-semibold">Encomenda #{{ encomendaId }}</h2>
          <p class="text-gray-600">Cliente ID: {{ encomenda?.customerId }}</p>
          <p class="text-gray-600">Estado: <span :class="styleStatusBadge(encomenda?.estado)">{{ encomenda.estado }}</span></p>
        </div>
      </div>
            ...
            <table class="min-w-full">
              ...
              <tr v-for="sensor in sensors">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span :class="styleStatusBadge(sensor.status)">{{ getEstadoSensor(sensor.status) }}</span>
                </td>
              </tr>
              </tbody>
            </table>
